...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(timedelta(minutes=16), 12, 1, id="16-minutes"),
        pytest.param(timedelta(minutes=59, seconds=59), 12, 1, id="59m59s"),
        pytest.param(timedelta(hours=1), 12, 1, id="1-hour"),
        pytest.param(timedelta(hours=1, minutes=15, seconds=59), 12, 1, id="1h15m59s"),
        pytest.param(timedelta(hours=1, minutes=16), 24, 2, id="1h16m"),
        pytest.param(timedelta(hours=2), 24, 2, id="2-hours"),
    ],
)
def test_basic_hourly_billing_boundaries(tmp_path, duration, expected_cost, expected_hours):
    ...
    assert sum(segment["rawCost"] for segment in result["segments"]) == expected_hours * 12


@pytest.mark.parametrize(
    ("entry_at", "expected_rule", "expected_cost", "first_segment_minutes"),
    [
        pytest.param(datetime(2026, 8, 24, 11, 29), "夜晚计费", 25, 31, id="entry-11h29"),
        pytest.param(datetime(2026, 8, 24, 11, 30), "白天计费", 12, 60, id="entry-11h30"),
        pytest.param(datetime(2026, 8, 24, 23, 0), "白天计费", 12, 60, id="entry-23h00"),
        pytest.param(datetime(2026, 8, 24, 23, 29), "白天计费", 12, 60, id="entry-23h29"),
        pytest.param(datetime(2026, 8, 24, 23, 30), "白天计费", 12, 60, id="entry-23h30"),
        pytest.param(datetime(2026, 8, 24, 23, 59), "白天计费", 12, 60, id="entry-23h59"),
    ],
)
...
def test_overnight_cap_covers_late_entry_first_hour(tmp_path):
    ...
    assert result["totalCost"] == 69
    assert result["overnightCap"]["cappedCost"] == 69
    assert result["overnightCap"]["rawCost"] == 81
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(timedelta(hours=24), 99, 1, None, id="continuous-24-hour-cap"),
        pytest.param(timedelta(hours=25), 111, 2, 12, id="second-block-after-24-hours"),
    ],
)
def test_continuous_24_hour_blocks(
    ...
    async def run():
        ...
        assert result["billing"]["blocks"][0]["cappedCost"] == 99
